...
void waitfg(pid_t pid)
{
    /*
    pid_t retpid;
    if ((retpid = waitpid(pid, NULL, WUNTRACED)) > 0)
    {
        printf("in waitfg, ready to delete pid %d", pid);
        deletejob(jobs, pid);
    }
    else
    {
...
//    printf("in the sigchlid\n");
    ...
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0)
    {
        ...
        {
            //printf("delete some pid\n");

            deletejob(jobs, pid);
        }
    }
    //printf("leave sigchld\n");
    return;
}
